...
export default function Catalogue() {
    ...
    // Create state for expanding filters
    ...
    // Clear the filter when a button is clicked
                ...
                <div className={styles.catalogueInner}>
                    <div className={styles.top}>
                        <div className={styles.filterTop}>
                            <h3 className={mainStyles.title} style={{ textTransform: 'capitalize' }}>
                                Filtering:
                            </h3>
                            <div className={styles.selectedFilters}>
                                {Object.entries(selectedFilters).map(([filterKey, filterValue]) => (
                                    <div
                                        ...
                                        onClick={() => cleanFilter(filterKey)}
                                    >
                                        <span className={`${styles.filterKey} ${styles.filteredKey}`}>
                                            {filterValue}
                                        </span>
                                        <svg
                                        ...
                                        >
                                            <path
                                                d="M11.4336 10.5L18.5088 17.5854L17.5859 18.5083L10.5005 11.4331L3.41504 18.5083L2.49219 17.5854L9.56738 10.5L2.49219 3.41455L3.41504 2.4917L10.5005 9.56689L17.5859 2.4917L18.5088 3.41455L11.4336 10.5Z"
                                                fill="#F3F3F3"
                                            />
                                        </svg>
                                    </div>
                        ...
                        <div className={styles.filters}>
                            {FiltersData.map((obj, index) => (
                                <div
                                    key={index}
                                    onClick={() => openingFilter(obj)}
                                    className={styles.filter}
                                >
                                    <p className={styles.filterTitle}>{obj.filter}</p>
                                    <svg
                                        className={`${styles.filterIcon} ${
                                            openedFilter == obj.filter ? styles.filterIconActive : ''
                                        }`}
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="12"
                                        height="7"
                                    ...
                                    <ul
                                        className={`${styles.filterInner} ${
                                            openedFilter == obj.filter ? styles.filterActive : ''
                                        }`}
                                    >
                                        {obj.items.map((item, index) => (
                                            <li
                                                key={index}
                                                onClick={() => handleFilter(obj.filter, item.key)}
                        ...
                        <div className={styles.searching}>
                            <input
                                ...
                                placeholder="Find the desired model"
                            />
                            <svg
                                onClick={cleanSearch}
                                className={`${styles.searchIcon} ${
                                    searchValue == '' ? '' : styles.searchIconActive
                                }`}
                                xmlns="http://www.w3.org/2000/svg"
                                width="17"
                                height="17"
                            ...
                            >
                                <path
                                    d="M9.43359 8.5L16.5088 15.5854L15.5859 16.5083L8.50049 9.43311L1.41504 16.5083L0.492188 15.5854L7.56738 8.5L0.492188 1.41455L1.41504 0.491699L8.50049 7.56689L15.5859 0.491699L16.5088 1.41455L9.43359 8.5Z"
                                    fill="#F3F3F3"
                                />
                            </svg>
                        </div>
                        ...
                        {currentItems.length >= 1 ? (
                            ...
                            <div className={styles.watchesClean}>
                                <p className={mainStyles.text}>
                                    Unfortunately, no watches matching the specified filters were found in our catalog. Try changing the filter parameters or returning to the full list to find suitable watches.
                                </p>
                            </div>
                        )}
                    </div>
                    <div className={styles.paginations}>
                        <span onClick={prevPage} className={`${styles.pagination} ${styles.pagePagination}`}>
                            Previous
                        </span>
                        <img
                            onClick={prevPage}
                            className={styles.paginationArrow}
                            src="./rich-tales/arrow-left.svg"
                            alt=""
                        />
                        ...
                        <img
                            onClick={nextPage}
                            className={styles.paginationArrow}
                            src="./rich-tales/arrow-right.svg"
                            alt=""
                        />
                        <span onClick={nextPage} className={`${styles.pagination} ${styles.pagePagination}`}>
                            Next
                        </span>
                    </div>
                </div>
